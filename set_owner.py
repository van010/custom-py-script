...
def run_set_owner(user_, path_):
    ...
    if options == 1:
        ...
        if owner == '':
            utils.run_command(f'sudo -S chown -R www-data.{cfg.facl_user} {path}')
        else:
            utils.run_command(f'sudo -S chown -R {owner}.{cfg.facl_user} {path}')
        print('Set owner Success!\n')
        utils.run_command(f'ls -l {path_list_chown}')
    else:
        user = str(input(f'Enter an User for setfacl <empty for {cfg.facl_user}>: '))
        if user == '':
            utils.run_command(f'sudo -S setfacl -R -m u:{cfg.facl_user}:wxr {path}')
        else:
